$(document).bind("mobileinit", function(){
  ...
  $("#send_message_button").live('click',function(){
    $.ajax({
      type: "POST",
      url: 'http://api.wibble.bibble:3000/pools/'+$("#pool_id").val()+'/sendmessage.js',
      data: {
       "token" : users_token(),
  ...
  $("#submit_carpool_button").live('click',function(){
    ...
    $.ajax({
      type: "POST",
      url: 'http://api.wibble.bibble:3000/pools.js',
      data: {
       "token" : users_token(),
  ...
  $("#sign_in_button").live('click',function(){
    $.ajax({
      type: "POST",
      url: 'http://api.wibble.bibble:3000/tokens.js',
      data: {
  ...
  $("a.pool_link").live('click',function(e){
    $("#pool_details").empty();
    $.getJSON("http://api.wibble.bibble:3000/pools/"+$(this).data('identity')+".js?callback=?", function(data) {
  ...
  $("a.country_click").live('click',function(){
    ...
    $.getJSON("http://api.wibble.bibble:3000/countries/"+window.localStorage.getItem("chosenCountry")+"/fields.js", function(data) {
...
function empty_and_refresh_carpools(){
  $("#append-pools").empty();
  $.getJSON("http://api.wibble.bibble:3000/countries/"+window.localStorage.getItem("chosenCountry")+".js?callback=?", function(data) {
    if(data.length == 0){
